...
import 'package:todoapp/models/breaktime_model.dart';
...
class DataBaseImpl {
  Future<Database> database() async {
    return openDatabase(
      ...
      onCreate: (db, version) async {
        ...
        await db.execute(
            "CREATE TABLE breaktimeTable(id INTEGER PRIMARY KEY, time INTEGER)");
      },
      version: 1,
    );
  }
  ...
  Future<int> insertTask(TaskModel taskModel) async {
    ...
    await _db
        .insert('taskTable', taskModel.toMap(),
  ...
  Future<void> updateTaskTitle(int? id, String? title) async {
    Database _db = await database();
    await _db
        .rawUpdate("UPDATE taskTable SET title = '$title' WHERE id = '$id'");
  }

  Future<void> updateTaskDesc(int? id, String? desc) async {
    Database _db = await database();
    await _db.rawUpdate(
        "UPDATE taskTable SET description = '$desc' WHERE id = '$id'");
  }

  Future<void> updateIsDone(int? id, int? isDone) async {
    Database _db = await database();
    await _db
        .rawUpdate("UPDATE todoTable SET isDone = '$isDone' WHERE id = '$id'");
  }

  Future<void> insertTodo(TodoModel todoModel) async {
    Database _db = await database();
    await _db.insert('todoTable', todoModel.toMap(),
        conflictAlgorithm: ConflictAlgorithm.replace);
  }
  ...
  Future<void> insertBreakTime(int time) async {
    Database _db = await database();
    await _db.insert("breaktimeTable", BreakTimeModel(time: time).toMap(),
        conflictAlgorithm: ConflictAlgorithm.replace);
  }

  Future<int> getBreakTime() async {
    Database _db = await database();
    List<Map<String, dynamic>> timeMap =
        await _db.rawQuery("SELECT * FROM breaktimeTable WHERE id = 1");

    return timeMap[0]['time'];
  }
}
